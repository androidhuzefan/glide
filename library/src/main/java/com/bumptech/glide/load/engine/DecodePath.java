...
public class DecodePath<DataType, ResourceType, Transcode> {
  ...
  public Resource<Transcode> decode(
    ...
    // 这个方法是回调到Decodejob.onResourceDecoded ,作用是调用RequestOptions中的Transform处理图片，
    // 然后将ResourceCache的Key和Encode准备好（放在变量 deferEncoderManager中），稍后进行写入缓存。
    ...
    for (int i = 0, size = decoders.size(); i < size; i++) {
      ...
      try {
        ...
        if (decoder.handles(data, options)) {
          ...
          // decode是一个ResourceDecoder接口（资源解码器）,
          // 根据不同的DataType和ResourceType它会有不同的实现类，这里的实现类是ByteBufferBitmapDecoder
